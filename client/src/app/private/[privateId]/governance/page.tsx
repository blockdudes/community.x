...
export default function Governance() {
    ...
    useEffect(() => {
        if (isAuthenticated && socketRef.current) {
            socketRef.current.emit("join_space", { space: "private", privateSpaceId: params.privateId, channel: "governance" });
        ...
        try {
            if (isAuthenticated && socketRef.current) {
                if (titleInput && descriptionInput) {
                    if ((creator as any).address === "0xd9eb5cfed425152a47a35dcfc43d0acbfb865feba0fc54f20fc6f40903c467d6") {
                        console.log("iwjneirjnfierjnierjn")
                        let post = "";
                        if (fileInput) {
